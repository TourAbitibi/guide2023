...
```{r include=FALSE}
dim_debut <- format(iti_etape$Details$Date[1]- ddays(2) ,format = "%d %B", locale = locale("en"))
lun <- format(iti_etape$Details$Date[1]- ddays(1) ,format = "%d %B", locale = locale("en"))
mar <- format(iti_etape$Details$Date[1], format = "%d %B", locale = locale("en"))
mer <- format(iti_etape$Details$Date[2], format = "%d %B", locale = locale("en"))
dim_fin <- format(iti_etape$Details$Date[7] ,format = "%d %B %Y", locale = locale("en"))

```

Article 4 – Permanence

:   The race headquarters are at the La Calypso high school, from Sunday `r dim_debut` to sunday `r dim_fin`, located at 800 1re Rue E, Amos, QC J9T 2H8. Opening hours are established in function of the race schedule, but there is a security guard on duty 24 hours a day.

    Initial meeting between the Tour’s organisation and the team manager must be done at the headquarters on Sunday `r dim_debut`, between 12:00 PM and 11:00 PM and Monday `r lun`, between 8:00 AM and 4:30 PM.
    
    Licence checks by the Commissaires’ Panel, identification numbers and frame plates delivery will be done on Monday `r lun` between 2:00 PM and 3:30 PM.
    
    Under the UCI rules and regulations article 1.2.087, the mandatory sports directors’ meeting with the Commissaires’ Panel is on Tuesday `r mar` at 10:00 AM.
...
:   The starting order of the individual time trial is the reverse order of the individual general classification after stage 2. Commissaires can modify the order to avoid two successive riders from the same team (2.6.023). 
    ...
    An optional bike check will be offered by the commissaires on Wednesday `r mer` at 10:30 AM.
...
df %>% 
  ...
  kbl(
    ...
    row_spec(which(df$Postion == "Total"), 
             ...
             color = couleurs$blueTour,
             font_size = "larger") %>% 
  add_header_above(c("Individual General Time Classification" =2),
                   font_size = "x-large",
...
df %>% 
  ...
  kbl(
    ...
    row_spec(which(df$Postion == "Total"), 
             ...
             color = couleurs$blueTour,
             font_size = "larger") %>% 
  add_header_above(c("General King of the Mountains Classification" =2),
                   font_size = "x-large",
...
df %>% 
  ...
  kbl(
    ...
    row_spec(which(df$Postion == "Total"), 
             ...
             color = couleurs$blueTour,
             font_size = "larger") %>% 
  add_header_above(c("Individual General Time Classification - First year junior" =2),
                   font_size = "x-large",
...
df %>% 
  ...
  kbl(
    ...
    row_spec(which(df$Postion == "Total"), 
             ...
             color = couleurs$blueTour,
             font_size = "larger") %>% 
  add_header_above(c("Team challenge" =2),
                   font_size = "x-large",
