...
subtitle: "Détails préliminaires"
...
output:
  html_document:
    ...
    includes:
      in_header: matamo-analytics.html
params:
  annee: 2023
  ville: "Amos"
  sponsor: "Glencore"
...
# Tour de l'Abitibi `r params$sponsor`

Voici un aperçu préliminaire de la `r edition`e édition du Tour de l'Abitibi `r params$sponsor` qui se tiendra du `r dates_FR %>% filter(code=="Mar") %>% pull(jsem_jour_mois_an)` au `r dates_FR %>% filter(code=="Dim") %>% pull(jsem_jour_mois_an)` à `r params$ville`. 
...
De 16:30 à 19:00, secteur Cathédrale. 
